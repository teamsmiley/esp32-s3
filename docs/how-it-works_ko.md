...
플로트는 **12인치(30.48cm) 높이**이고, MS5837 depth 센서는 **6인치 미들(midpoint)**에 마운트되어 있습니다.

```text
   ━━━━━ ← top
  ┃ 6   ┃
  ┃ in  ┃
  ┃  ●  ┃ ← 센서 (미들, bottom - 0.1524m을 읽음)
  ┃ 6   ┃
  ┃ in  ┃
   ━━━━━ ← bottom (= 센서값 + 0.1524 m)
```
...
| 함수              | 반환값                         | 용도                                            |
| ----------------- | ------------------------------ | ----------------------------------------------- |
| `reportedDepth()` | 센서값 + 0.1524m (플로트 하단) | 모든 HOLD 밴드, SURFACE 검사, 패킷 출력, 그래프 |

미션이 **혼합 기준**으로 hold 목표를 명시(2.5m는 bottom, 40cm는 top)하지만, 내부적으로는 모든 것을 bottom 깊이(`reportedDepth()`)로 정규화해 일관된 밴드 비교를 합니다. SURFACE 검사도 bottom 기준(`reportedDepth() ≤ 0.20 m`) — 미들 마운트 덕분에 센서가 HOLD_SHALLOW 밴드 전체에서 항상 물에 잠겨 있어, 탑 마운트라면 0m에서 saturation되어 잃어버릴 깊이 신호가 연속적으로 유지됩니다.
...
`MOTOR_SPEED_HOLD`는 부팅 시 `/cali.txt`에서 로드. 파일이 없거나, 형식이 잘못됐거나, 1–255 범위를 벗어나면 하드코딩된 기본값(90/255)으로 fallback. `C` 명령은 수중에서 PWM sweep을 돌려 deep 밴드에서 가장 드리프트가 작은 PWM을 찾아 저장. `X`로 중간 abort 시 `/cali.txt`는 변경되지 않음.
