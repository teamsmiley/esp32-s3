...
플로트는 **12인치(30.48cm) 높이**이고, MS5837 depth 센서는 **바닥에서 약 0.5인치(1.27cm) 위**에 마운트되어 있습니다 (하우징을 위한 작은 standoff).
...
```text
  ...
  ┃ in  ┃
  ┃     ┃
  ┃  ●  ┃ ← 센서 (바닥에서 0.5in 위, bottom - 0.0127m을 읽음)
   ━━━━━ ← bottom (= 센서값 + 0.0127 m)
```
...
| 함수              | 반환값                         | 용도                                            |
| ----------------- | ------------------------------ | ----------------------------------------------- |
| `reportedDepth()` | 센서값 + 0.0127m (플로트 하단) | 모든 HOLD 밴드, SURFACE 검사, 패킷 출력, 그래프 |
