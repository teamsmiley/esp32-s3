...
PlatformIO Core 는 `tools/` 의 uv 가상환경 안에 들어있습니다 (`tools/pyproject.toml` 의 `platformio` 의존성). 시스템 전역 `pio` 가 따로 깔려있을 필요 없음.

**활성화 흐름**: 터미널마다 한 번씩 `cd tools && source .venv/bin/activate` (Windows: `.venv\Scripts\Activate.ps1`) 하면 그 후로는 `pio` 를 직접 호출 가능. 사용자 안내 명령은 활성화된 상태를 가정해서 `pio ...` 만 적으세요.

**중요**: 펌웨어가 두 개로 분리되어 있습니다 (`float/`, `station/`). 보드 디렉토리는 `-d ../<board>` 옵션으로 지정.

| 명령어                                          | 용도                                                            |
| ----------------------------------------------- | --------------------------------------------------------------- |
| `pio run -d ../float -t upload -t monitor`     | **플로트 보드 메인 개발 루프**                                  |
| `pio run -d ../station -t upload -t monitor`   | **지상국 보드 메인 개발 루프**                                  |
| `pio run -d ../<board>`                         | 빌드만                                                          |
| `pio device monitor --baud 115200`              | 시리얼 모니터만 — `-d` 없이도 포트 자동 탐지                    |
| `pio device list`                               | 사용 가능한 시리얼 포트 목록                                    |
| `pio run -d ../<board> -t clean`                | 빌드 결과물 청소 (Flash/PSRAM 설정 변경 후 권장)                |

실제 운용에서는 한 번에 한 보드만 컴퓨터에 연결합니다 (float 과 station 을 동시에 같은 컴에 꽂는 경우 없음). 포트 자동 탐지로 충분.
...
- `float/` — **플로트 보드** 펌웨어 (깊이 센서 연결, 자율 프로파일 수행, 패킷 송신)
  - `float/src/main.cpp` — 플로트 메인 스케치
  - `float/platformio.ini` — 보드 설정 + MS5837 라이브러리 의존성
- `station/` — **지상국 수신기** 펌웨어 (ESP-NOW 수신, 시리얼 출력)
  - `station/src/main.cpp` — 수신기 메인 스케치
  - `station/platformio.ini` — 보드 설정 (라이브러리 의존성 없음)
- `docs/2026_MATE_Floats_분석.md` — 미션 규정 분석 (점수표, 패킷 형식, 물리·전기 제약)
- `examples/` — 과거 학습 단계의 최소 재현 코드 (LED, 버튼 등). PlatformIO 빌드 대상 아님
- `TODO.md` — 미션 진행 중 발견한 "나중에 할 것" 목록
