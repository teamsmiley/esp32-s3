...
**중요**: 펌웨어가 두 개로 분리되어 있습니다 (`float/`, `station/`). 빌드·업로드 명령은 **반드시 해당 디렉토리 안에서** 실행하세요.

| 명령어                                          | 용도                                                                              |
| ----------------------------------------------- | --------------------------------------------------------------------------------- |
| `cd float && pio run -t upload -t monitor`      | **플로트 보드 메인 개발 루프**                                                    |
| `cd station && pio run -t upload -t monitor`    | **지상국 보드 메인 개발 루프**                                                    |
| `pio run`                                       | 빌드만 (해당 디렉토리에서)                                                        |
...
두 보드를 동시에 컴퓨터에 연결한 경우, 어느 보드에 올릴지 명시:
```
cd float && pio run -t upload --upload-port /dev/cu.usbmodem143401
cd station && pio run -t upload --upload-port /dev/cu.usbmodem1234561
```
