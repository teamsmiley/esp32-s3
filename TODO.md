...
- [ ] **로컬 데이터 로깅 (LittleFS)** — 미션 점수 ⑤ (회수 후 전송, 10점) + ⑥ (수심-시간 그래프, 10점) 의 토대
  - 5초마다 패킷을 16MB Flash 의 LittleFS 에 누적 저장
  - 회수 후 `CMD_DUMP_LOG` 로 일괄 송신
  - 그래프용 데이터 포인트 최소 20개 확보
...
- [ ] **무선 명령 처리** (양방향 ESP-NOW)
  - `CMD_ZERO` — 깊이 0점 재보정 (BOOT 버튼은 물속에서 못 누름. 배포 직후 수면에서 무선으로 보정)
  - `CMD_PING` — 연결 확인 / 응답 시간 측정
  - `CMD_DUMP_LOG` — 회수 후 로컬 로그 전체 송신 트리거
  - `CMD_START_MISSION` — 자율 시퀀스 시작 트리거
