...
- [ ] **수심-시간 그래프 도구** — 미션 점수 ⑥ (10점)
  - 도구 자체는 완성 (`tools/capture_and_graph.py`, 완료 섹션 참고)
...
- [x] **수심-시간 그래프 도구** (`tools/capture_and_graph.py`) — 미션 점수 ⑥ 의 토대
  - Python + pyserial + matplotlib (`tools/pyproject.toml` + `uv.lock`, uv 로 격리)
  - station USB 시리얼 청취 → `[RX] PVPHSROV ...` 정규식 파싱 → CSV append
  - Ctrl+C 종료 시 matplotlib 으로 PNG 자동 저장 (Y축 반전, 데이터 포인트 수 표시)
  - 옵션:
    - `--send-cmd DUMP` 접속 직후 명령 자동 송신 (시연 자동화)
    - `--replot` 시리얼 없이 기존 CSV → PNG 재생성
  - 샘플 21 포인트 CSV 로 빈 그래프 → 정상 생성 확인. 실데이터 리허설은 미션 위쪽 TODO
